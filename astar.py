...
visited = []
solution = []
def Astar(graph, straight_distance, origin = "arad", destination = "arad"):
    ...
    if origin in graph and destination in graph:
        ...
        distance_list = []
        for n in neighbours:
            distance_list.append((n, straight_distance[n]+graph[origin][n])) #update distance to weigh in distance to next node as well
        distance_list.sort(key=lambda y: y[1])
        for (n,_) in distance_list:
            if (n not in visited):
                if Astar(graph, straight_distance, n, destination) == 1:
                    # if return 1 -> destination found -> add self as path to solution ->  signal to retroactively add caller
                    solution.append(origin)
                    return 1
    return None

Astar(graph, straight_distance_arad_bucharest)
solution.reverse()
cost = 0
current = None
...
print(f"astar solution pathway = {solution}, total cost: {cost}")